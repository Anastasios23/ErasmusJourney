...
function ensureConsoleNinjaStartupPlaceholders() {
  const nextDir = resolve(".next");
  const serverPagesDir = resolve(nextDir, "server", "pages");
  ...
  const fallbackChunksDir = resolve(nextDir, "static", "chunks", "fallback");
...
`;
  ...
  writeFileSync(resolve(fallbackPagesDir, "_error.js"), fallbackChunkStub, "utf8");
}

if (process.env.SKIP_NEXT_PLACEHOLDERS !== "true") {
  ensureConsoleNinjaStartupPlaceholders();
}
