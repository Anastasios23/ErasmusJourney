...
export default async function handler(
  ...
  try {
    const { city, country, all, enhanced } = req.query;
    ...
    try {
      // Choose between enhanced or basic data based on query parameter
      const cityData =
        enhanced === "true"
          ? await getEnhancedCityData(city as string, country as string)
          : await aggregateCityData(city as string, country as string);
      ...
      console.error(
        `Error in aggregation for ${city}, ${country}:`,
        aggregationError,
      );
