  ...
  const fetchCourseMatchingExperiences = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        // For 500 errors, we can still show the page with empty data
        if (response.status === 500) {
          ...
          // Don't throw - let the page render with empty state
          return;
        }
        throw new Error(
          errorData.error || "Failed to fetch course matching experiences",
        );
      }
      ...
      // Set empty state instead of error to allow page to render
      setExperiences([]);
      setFilterStats({
        totalExperiences: 0,
        avgDifficulty: 0,
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          ...
          >
            ...
            <Link href="/share-experience">
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center gap-2 px-4 py-2 bg-white text-indigo-600 rounded-full text-sm font-medium hover:bg-white/90 transition-colors shadow-lg"
              >
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4 overflow-hidden">
            <span className="flex flex-wrap gap-x-3">
              {["Course", "Matching", "Experiences"].map((word, index) => (
                <motion.span
                  key={index}
