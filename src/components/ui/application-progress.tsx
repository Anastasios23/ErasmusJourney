  ...
  if (compact) {
    return (
      <div className={cn("flex items-center space-x-2", className)}>
        ...
        <span className={cn("text-xs font-medium", brandingTokens.colors.text.tertiary)}>
  ...
  return (
    <div className={cn("w-full", className)}>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className={cn("text-sm font-medium", brandingTokens.colors.text.primary)}>
        ...
        <div className={cn("text-sm font-semibold", brandingTokens.colors.text.accent)}>
          ...
          return (
            ...
            >
              ...
              <div className={cn(
                ...
                isCurrent && !isCompleted && "bg-blue-100 dark:bg-blue-900/30 ring-2 ring-blue-500 ring-offset-2 ring-offset-white dark:ring-offset-gray-900",
                isLocked && "bg-gray-100 dark:bg-gray-700",
                !isCurrent && !isCompleted && !isLocked && "bg-gray-100 dark:bg-gray-700"
              )}>
                {isCompleted ? (
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center space-x-2">
                  <span className={cn(
                    ...
                    isCurrent && !isCompleted && brandingTokens.colors.text.accent,
                    isLocked && brandingTokens.colors.text.muted,
                    !isCurrent && !isCompleted && !isLocked && brandingTokens.colors.text.secondary
                  )}>
                    {step.name}
                  </span>
                  {isCompleted && (
                ...
                <p className={cn(
                  "text-xs mt-0.5",
                  isLocked ? brandingTokens.colors.text.muted : brandingTokens.colors.text.tertiary
                )}>
                  {step.description}
                </p>
              </div>
...
export function createApplicationSteps(
  ...
  stepKeys.forEach((stepId, index) => {
    ...
    const previousStepsCompleted = index === 0 || steps.slice(0, index).every(s => s.completed);
